...
exports.create = async (req, res) => {
  try {
    ...
    if (payload.role === 'ADMIN') {
      const existing = await StaffAdmin.findOne({ role: 'ADMIN', includeDisabled: true });
      if (existing) return res.status(400).json({ success:false, error: 'ADMIN already exists' });
    }
...
exports.getById = async (req, res) => {
  try {
    const doc = await StaffAdmin.findById(req.params.id, req.query.includeDisabled ? { includeDisabled: true } : {});
...
exports.update = async (req, res) => {
  try {
    ...
    const doc = await StaffAdmin.findByIdAndUpdate(req.params.id, updates, { new: true });
    if (!doc) return res.status(404).json({ success:false, error: 'not found' });
    return res.json({ success:true, data: doc });
...
exports.restore = async (req, res) => {
  try {
    const doc = await StaffAdmin.findOne({ _id: req.params.id, includeDisabled: true });
    if (!doc) return res.status(404).json({ success:false, error: 'not found' });
